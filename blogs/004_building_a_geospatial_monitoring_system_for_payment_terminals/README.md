...
- `003_load_boundaries.sh` - loads the LGA and world country boundary shapefiles from
  `src-data/` into `lga_boundaries` and `country_boundaries`, reprojecting both to
  EPSG:4326 for GeoServer to publish as base map layers
...
- `src-data/` - source datasets (cell towers, LGA and country boundaries) used to seed
  PostGIS
